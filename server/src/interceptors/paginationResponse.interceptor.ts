...
@Injectable()
export class PaginationResponse implements NestInterceptor {
	intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
		return next.handle().pipe(
			map(response => ({
				meta: {
					page: response.pagination?.page + 1 || 1,
					limit: response.pagination?.limit || null,
					...
					totalPages: Math.ceil(
						response.pagination?.totalResults / response.pagination?.limit,
					),
				},
